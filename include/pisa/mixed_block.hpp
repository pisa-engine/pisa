...
struct mixed_block {
    ...
    {
        switch (t) {
        case block_type::pfor: return optpfor_block::codec_type::possLogs.size();
        default: return 1;
        }
    }
        ...
        std::vector<uint8_t>& out)
    {
        ...
        case block_type::varint: varint_G8IU_block::encode(in, sum_of_values, n, out); break;
        case block_type::interpolative:
            interpolative_block::encode(in, sum_of_values, n, out);
            break;
        default: throw std::runtime_error("Unsupported block type");
        }
    }

    static bool compression_stats(
        block_type type,
        compr_param_type param,
        uint32_t const* in,
        uint32_t sum_of_values,
        ...
        for (uint8_t t = 0; t < block_types; ++t) {
            ...
            for (compr_param_type param = 0; param < compr_params(type); ++param) {
                ...
                if (!compression_stats(
                        type, param, values.data(), sum_of_values, values.size(), buf, fv)) {
                    continue;
                }
    ...
    struct block_transformer {
        ...
        void append_freqs_block(std::vector<uint8_t>& out) const
        {
            ...
            encode_type(m_freqs_type, m_freqs_param, buf.data(), uint32_t(-1), size, out);
        }

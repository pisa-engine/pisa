...
namespace {
    ...
    static const size_t ls_bits_size = broadword::msb(configuration::get().ls_max_value) + 1;
}

    class uniform_score_compressor_ls{
    ...
    public:
        ...
        public:
            builder(uint64_t num_docs, global_parameters const& params)
                : m_params(params)
                , m_num_docs((num_docs+1) << (score_bits_size + ls_bits_size))
            ...
            std::vector<uint32_t> find_ls(std::vector<float> effective_scores){
            
                std::vector<uint32_t> long_skips;
                long_skips.reserve(effective_scores.size());
                for (int i = 0; i < effective_scores.size(); ++i){
                    uint32_t next_pos = 0;
                    for (next_pos = 1; next_pos <= configuration::get().ls_max_value && i + next_pos < effective_scores.size(); ++next_pos){
                        if (effective_scores[i + next_pos] > effective_scores[i]){
                            break;
                        }
                    }
                    next_pos--;
                    long_skips.push_back(next_pos);
                }
            ...
            void add_posting_list(uint64_t n, DocsIterator docs_begin, DocsIterator score_begin, DocsIterator ls_begin)
            {
    ...
    template<typename Scorer = bm25, typename score_compressor = uniform_score_compressor_ls>
    class wand_data_compressed_ls {
    public:
        class builder{
        public:
            ...
            float add_sequence(binary_freq_collection::sequence const &seq, binary_freq_collection const &coll, std::vector<float> const & norm_lens, const float lambda = 0.0f){
                ...
                if (seq.docs.size() > configuration::get().threshold_wand_list) {
                    ...
                    auto long_skips = compressor_builder.find_ls(t.second);

                    compressor_builder.add_posting_list(t.first.size(), t.first.begin(),
                                                           ind.begin(), long_skips.begin());
            ...
            void build(wand_data_compressed_ls & wdata){
        ...
        class enumerator{
            friend class wand_data_compressed_ls;
        public:
            enumerator(compact_elias_fano::enumerator docs_enum)
                : m_docs_enum(docs_enum), m_long_skip_index(0), ls_next_docid_valid(false)
            {
                reset();
            }
            ...
            void reset()
            {
                ...
                m_cur_docid = val >> (score_bits_size + ls_bits_size);
                uint64_t mask = configuration::get().reference_size - 1;
		        m_cur_score_index = ((val >> ls_bits_size) & mask);
            }

            void PISA_FLATTEN_FUNC next_geq(uint64_t lower_bound) {
                if(docid() != lower_bound) {
                    ls_next_docid_valid = false;
                    lower_bound = lower_bound << (score_bits_size + ls_bits_size);
                    auto val = m_docs_enum.next_geq(lower_bound);
                    m_cur_docid = val.second >> (score_bits_size + ls_bits_size);
		            uint64_t mask = configuration::get().reference_size - 1;
                    m_cur_score_index = ((val.second >> ls_bits_size) & mask);
                    uint64_t mask_ls = configuration::get().ls_max_value;
                    m_long_skip_index = (val.second & mask_ls);
                }
            }

            // find_next_docid returns the docid belonging to the next block with a score grater than the current one.
            uint64_t inline find_next_docid() {
                if (!ls_next_docid_valid) {
                    ls_next_docid_valid = true;
                    auto val = m_docs_enum.skip_no_move(m_long_skip_index);
                    ls_next_docid = val >> (score_bits_size + ls_bits_size);
                }

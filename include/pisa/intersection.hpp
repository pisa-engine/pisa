...
struct Intersection {
    ...
    inline static auto compute(Index const &index,
                               Scorer const &scorer,
                               Query const &query,
...
template <typename Index, typename Scorer>
inline auto Intersection::compute(Index const &index,
                                  Scorer const &scorer,
                                  Query const &query,
                                  std::optional<intersection::Mask> term_mask) -> Intersection
{
    ...
    auto cursors = make_scored_cursors(index, scorer, filtered_query);
    auto results = retrieve(gsl::make_span(cursors), index.num_docs());

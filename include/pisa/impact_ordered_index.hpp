...
#include "segment_posting_list.hpp"

namespace pisa {

template <typename BlockCodec>
class impact_ordered_index;

template <typename>
struct is_impact_ordered_index : std::false_type {
};

template <typename C>
struct is_impact_ordered_index<impact_ordered_index<C>> : std::true_type {
};

template <typename BlockCodec>
class impact_ordered_index {
   public:
    impact_ordered_index() : m_size(0) {}
    ...
    class builder {
       public:
                              ...
                              uint64_t /* occurrences */)
        {
            ...
            segment_posting_list<BlockCodec>::write(m_lists, n, docs_begin, freqs_begin);
            m_endpoints.push_back(m_lists.size());
        }

        void build(impact_ordered_index &sq)
        {
    ...
    typedef typename segment_posting_list<BlockCodec>::segment_enumerator segment_enumerator;

        segment_enumerator operator[](size_t i) const
        {
            assert(i < size());
            ...
            return segment_enumerator(m_lists.data() + endpoint, num_docs(), i);
        }
    ...
    void swap(impact_ordered_index &other)
    {

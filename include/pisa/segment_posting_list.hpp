...
namespace pisa {
    ...
    struct segment_posting_list {
        ...
        static void write(std::vector<uint8_t>& out, uint32_t n,
            ...
            std::vector<float> scores_buf(n);

            for (size_t i = 0; i < n; ++i) {
                uint32_t doc(*docs_it++);
                docs_buf[i] = doc;

                scores_buf[i] = *freqs_it++;
            //     last_doc = doc;
            }
        ...
        class segment_enumerator {
        public:

            segment_enumerator(uint8_t const* data, uint64_t universe,
                                size_t term_id = 0)

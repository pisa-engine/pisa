...
struct wand_query {
    ...
    template <typename CursorRange>
    void twopass(CursorRange&& cursors, uint64_t start_id, uint64_t max_docid)
    {
        using Cursor = typename std::decay_t<CursorRange>::value_type;
        if (cursors.empty()) {
            return;
        ...
        ordered_cursors.reserve(cursors.size());
        for (auto& en: cursors) {
            en.next_geq(start_id); // XXX Forward to start point
            ordered_cursors.push_back(&en);
        }
        ...
        while (true) {
            ...
            bool found_pivot = false;
            for (pivot = 0; pivot < ordered_cursors.size(); ++pivot) {
                if (ordered_cursors[pivot]->docid() >= start_id) {
                    break;
                }
                upper_bound += ordered_cursors[pivot]->max_score();
                if (m_topk.would_enter(upper_bound)) {

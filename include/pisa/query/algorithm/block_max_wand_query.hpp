...
struct block_max_wand_query {
    ...
    void operator()(CursorRange&& cursors, uint64_t max_docid, Inspect* inspect = nullptr)
    {
        ...
        //                score += part_score;
        //                block_upper_bound -= en->block_max_score() * en->query_weight() -
        //                part_score; if (!m_topk.would_enter(block_upper_bound)) {
        //                    break;
        //                }
        //            }
        //            for (Cursor* en: ordered_cursors) {
        ...
        //        next = next + 1;
        //        if (pivot + 1 < ordered_cursors.size() && ordered_cursors[pivot + 1]->docid() <
        //        next) {

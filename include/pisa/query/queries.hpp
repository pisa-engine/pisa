...
[[nodiscard]] inline auto parse_query_terms(std::string const &query_string,
                                            TermProcessor term_processor) -> Query
{
...
[[nodiscard]] inline auto parse_query_ids(std::string const &query_string) -> Query
{
    auto [id, raw_query] = split_query_at_colon(query_string);
    std::vector<term_id_type> parsed_query;
...
[[nodiscard]] inline auto resolve_query_parser(std::vector<Query> &queries,
...
inline auto read_query(term_id_vec &ret, std::istream &is = std::cin) -> bool
{
    ret.clear();
    std::string line;
...
[[nodiscard]] inline auto query_freqs(term_id_vec terms) -> term_freq_vec
{

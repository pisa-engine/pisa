...
template <typename Source = mio::mmap_source, typename Posting = std::uint32_t>
class base_binary_collection {
   public:
    static_assert(std::is_trivially_copyable<Posting>::value);
...
using binary_collection = base_binary_collection<mio::mmap_source>;

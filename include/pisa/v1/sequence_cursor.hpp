...
struct SequenceCursor {
    ...
    explicit constexpr SequenceCursor(gsl::span<std::byte const> encoded_blocks,
    ...
    constexpr SequenceCursor(SequenceCursor const&) = default;
    constexpr SequenceCursor(SequenceCursor&&) noexcept = default;
    constexpr SequenceCursor& operator=(SequenceCursor const&) = default;
    constexpr SequenceCursor& operator=(SequenceCursor&&) noexcept = default;
...
struct SequenceReader {
    ...
    [[nodiscard]] auto read(gsl::span<const std::byte> bytes) const -> SequenceCursor<Sequence>
    {
        // std::uint32_t length;
        ...
        // if constexpr (DeltaEncoded) {
        //    return SequenceCursor<Codec, DeltaEncoded>(
    ...
    {
        return EncodingId::Sequence | encoding_traits<Sequence>::encoding_tag::encoding();
    }
};
...
template <typename Sequence>
struct CursorTraits<SequenceCursor<Sequence>> {
    using Writer = SequenceWriter<Sequence>;
    using Reader = SequenceReader<Sequence>;
};

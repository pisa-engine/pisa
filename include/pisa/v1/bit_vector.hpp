...
class BitVectorBuilder {
   public:
    ...
    BitVectorBuilder(uint64_t size = 0, bool init = 0) : m_size(size)
    {
    ...
    void append(BitVectorBuilder const& rhs)
    {
        if (!rhs.size())
            return;
...
class BitVector {
   public:
    ...
    BitVector() = default;
    BitVector(gsl::span<storage_type const> bits, std::size_t size) : m_bits(bits), m_size(size) {}
    ...
    std::uint64_t const* data() const { return m_bits.data(); }

    struct enumerator {
        enumerator() : m_bv(0), m_pos(uint64_t(-1)) {}

        enumerator(BitVector const& bv, size_t pos) : m_bv(&bv), m_pos(pos), m_buf(0), m_avail(0)
        {
    ...
    struct unary_enumerator {
        ...
        unary_enumerator(BitVector const& bv, uint64_t pos)
        {
            m_data = bv.data();
            m_position = pos;
            m_buf = m_data[pos / 64];
   ...
   protected:
    gsl::span<std::uint64_t const> m_bits{};
    std::size_t m_size = 0;
};
...
//
//    BitVector() = default;
//    BitVector(gsl::span<std::uint64_t const> bits, std::size_t size) : m_bits(bits), m_size(size)
//    {} BitVector(BitVector const&) = default; BitVector(BitVector&&) noexcept = default;
//    BitVector& operator=(BitVector const&) = default;
//    BitVector& operator=(BitVector&&) noexcept = default;
//    ~BitVector() = default;
//
//    [[nodiscard]] inline auto size() const -> std::size_t { return m_size; }
//
//    [[nodiscard]] inline auto operator[](std::size_t pos) const -> bool
//    {
//        Expects(pos < m_size);
//        std::uint64_t block = pos / 64;
//        Expects(block < m_bits.size());
//        std::uint64_t shift = pos % 64;
//        return ((m_bits[block] >> shift) & 1U) != 0U;
//    }
//
//    [[nodiscard]] inline auto get_bits(uint64_t pos, uint64_t len) const -> std::uint64_t
//    {
//        Expects(pos + len <= size());
//        Expects(len <= 64);
//        if (len == 0U) {
//            return 0;
//        }
...
//
//    // same as get_bits(pos, 64) but it can extend further size(), padding with zeros
//    [[nodiscard]] inline auto get_word(std::uint64_t pos) const -> std::uint64_t
//    {
//        Expects(pos < size());
//        std::uint64_t block = pos / 64;
//        std::uint64_t shift = pos % 64;
//        std::uint64_t word = m_bits[block] >> shift;
//        if (shift > 0U && block + 1U < m_bits.size()) {
//            word |= m_bits[block + 1] << (64 - shift);
//        }
//        return word;
//    }
//
//    // unsafe and fast version of get_word, it retrieves at least 56 bits
//    [[nodiscard]] inline auto get_word56(std::uint64_t pos) const -> std::uint64_t
//    {
//        // XXX check endianness?
//        const char* ptr = reinterpret_cast<const char*>(m_bits.data());
//        return *(reinterpret_cast<uint64_t const*>(ptr + pos / 8)) >> (pos % 8);
//    }
//
//    [[nodiscard]] inline auto predecessor0(std::uint64_t pos) const -> std::uint64_t
//    {
//        Expects(pos < m_size);
//        std::uint64_t block = pos / 64;
//        std::uint64_t shift = 64 - pos % 64 - 1;
//        std::uint64_t word = ~m_bits[block];
//        word = (word << shift) >> shift;
//
//        std::uint64_t ret;
...
//    }
//
//    [[nodiscard]] inline auto successor0(std::uint64_t pos) const -> std::uint64_t
//    {
//        Expects(pos < m_size);
//        std::uint64_t block = pos / 64;
//        std::uint64_t shift = pos % 64;
//        std::uint64_t word = (~m_bits[block] >> shift) << shift;
//
//        std::uint64_t ret;
...
//    }
//
//    [[nodiscard]] inline auto predecessor1(std::uint64_t pos) const -> std::uint64_t
//    {
//        Expects(pos < m_size);
//        std::uint64_t block = pos / 64;
//        std::uint64_t shift = 64 - pos % 64 - 1;
...
//
//        std::uint64_t ret;
//        while (broadword::msb(word, ret) == 0) {
//            Expects(block);
//            word = m_bits[--block];
//        };
//        return block * 64 + ret;
//    }
//
//    [[nodiscard]] inline auto successor1(std::uint64_t pos) const -> std::uint64_t
//    {
//        Expects(pos < m_size);
//        std::uint64_t block = pos / 64;
//        std::uint64_t shift = pos % 64;
...
//    }
//
//    [[nodiscard]] inline auto data() const -> std::uint64_t const* { return m_bits.data(); }
//
//    struct enumerator {
//        enumerator() : m_pos(uint64_t(-1)) {}
//
//        enumerator(BitVector const& bv, size_t pos) : m_bv(&bv), m_pos(pos)
//        {
//            intrinsics::prefetch(std::next(m_bv->data(), m_pos / 64));
//            fill_buf();
//        }
//
...
//        }
//
//        inline auto take(size_t l) -> std::uint64_t
//        {
//            if (m_avail < l) {
//                fill_buf();
//            }
...
//        }
//
//        [[nodiscard]] inline auto position() const -> std::uint64_t { return m_pos; }
//
//       private:
//        inline void fill_buf()
//        {
...
//        }
//
//        BitVector const* m_bv = nullptr;
//        std::size_t m_pos;
//        std::uint64_t m_buf = 0;
//        std::size_t m_avail = 0;
//    };
//
//    struct unary_enumerator {
//        unary_enumerator() = default;
//        unary_enumerator(BitVector const& bv, uint64_t pos)
//        {
//            m_data = bv.data();
//            m_position = pos;
...
//        }
//
//        [[nodiscard]] auto position() const -> std::uint64_t { return m_position; }
//
//        uint64_t next()
//        {
//            std::uint64_t pos_in_word;
//            std::uint64_t buf = m_buf;
//            while (broadword::lsb(buf, pos_in_word) == 0) {
//                m_position += 64;
//                buf = m_data[m_position / 64];
//            }
//

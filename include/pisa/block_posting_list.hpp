    ...
    class document_enumerator {
       public:
        document_enumerator(uint8_t const *data,
            ...
            : m_n(0), // just to silence warnings
              ...
              m_blocks(ceil_div(m_n, codec.block_size())),
              m_block_maxs(m_base),
              ...
              m_codec(std::move(codec))
        {
            ...
            m_docs_buf.resize(m_codec.block_size());
            m_freqs_buf.resize(m_codec.block_size());
            reset();
        }
        ...
        {
            assert(pos >= position());
            uint64_t block = pos / m_codec.block_size();
        ...
        uint64_t position() const { return m_cur_block * m_codec.block_size() + m_pos_in_block; }
        ...
        uint64_t stats_freqs_size() const
        {
            ...
            static const uint64_t block_size = m_codec.block_size();
            ...
            for (size_t b = 0; b < m_blocks; ++b) {
                ...
                uint8_t const *freq_ptr =
                    m_codec.decode(ptr,
                                   ...
                                   cur_block_size);
                ptr = m_codec.decode(freq_ptr, buf.data(), uint32_t(-1), cur_block_size);
                bytes += ptr - freq_ptr;
            }
        ...
        struct block_data {
            ...
            explicit block_data(BlockCodec codec) : codec(std::move(codec)) {}
            ...
            {
                out.resize(size);
                codec.decode(docs_begin, out.data(), doc_gaps_universe, size);
            }

            void decode_freqs(std::vector<uint32_t> &out) const
            {
                out.resize(size);
                codec.decode(freqs_begin, out.data(), uint32_t(-1), size);
            }
        ...
        std::vector<block_data> get_blocks()
        {
            ...
            for (size_t b = 0; b < m_blocks; ++b) {
                ...
                uint8_t const *freq_ptr =
                    m_codec.decode(ptr, buf.data(), gaps_universe, cur_block_size);
       ...
       private:
        ...
        void PISA_NOINLINE decode_docs_block(uint64_t block)
        {
            ...
            m_freqs_block_data = m_codec.decode(block_data,
                                                m_docs_buf.data(),
        ...
        {
            uint8_t const *next_block = m_codec.decode(

...
template <typename PayloadCollection>
class base_binary_freq_collection {
   public:
    base_binary_freq_collection(const char *basename)
    ...
    struct sequence {
        ...
        typename PayloadCollection::const_sequence freqs;
    };
    ...
    class iterator : public std::iterator<std::forward_iterator_tag, sequence> {
       ...
       private:
        friend class base_binary_freq_collection;

        iterator(binary_collection::const_iterator docs_it,
                 typename PayloadCollection::const_iterator freqs_it)
        ...
        typename PayloadCollection::const_iterator m_freqs_it;
        sequence m_cur_seq;
    };

   private:
    binary_collection m_docs;
    PayloadCollection m_freqs;
    uint64_t m_num_docs;
};

using binary_freq_collection = base_binary_freq_collection<binary_collection>;

...
class TailyStats {
  public:
    ...
    [[nodiscard]] auto query_stats(pisa::QueryContainer const& query) const -> taily::Query_Statistics
    {
        std::vector<taily::Feature_Statistics> stats;
        auto const& term_ids = query.term_ids();
        if (!term_ids.has_value()) {
            throw std::runtime_error("Query is not parsed");
        }
        std::transform(
            term_ids->begin(), term_ids->end(), std::back_inserter(stats), [this](auto&& term_id) {
...
void taily_score_shards(
    ...
    std::vector<::pisa::QueryContainer> const& global_queries,
    VecMap<Shard_Id, std::vector<::pisa::QueryContainer>> const& shard_queries,
    std::size_t k,
    Fn func)
{

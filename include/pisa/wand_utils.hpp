...
std::pair<std::vector<uint32_t>, std::vector<float>> static_block_partition(
    BinaryFreqCollection::sequence const &seq, Scorer scorer, const uint64_t block_size)
{
    ...
    for (i = 0; i < seq.documents.size(); ++i) {
        ...
        if (i == 0 || (i / block_size) == current_block) {
            ...
            block_docid.push_back(*(seq.documents.begin() + i) - 1);
            block_max_term_weight.push_back(block_max_score);
            current_block++;
            block_max_score = std::max((float)0, score);
        }
    }
    block_docid.push_back(*(seq.documents.begin() + seq.documents.size() - 1));
    block_max_term_weight.push_back(block_max_score);
...
std::pair<std::vector<uint32_t>, std::vector<float>> variable_block_partition(
    ...
    BinaryFreqCollection::sequence const &seq,
    Scorer scorer,
    const float lambda)
{
    ...
    std::transform(seq.documents.begin(),
                   seq.documents.end(),
                   seq.frequencies.begin(),

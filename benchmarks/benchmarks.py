...
def build_indexes(basename, fast=False, check=False):
    ...
    env = os.environ.copy()
    env['QS_THREADS'] = str(MAX_THREADS) if fast else '0'
    ...
    if not fast:
        ...
        env['QS_THREADS'] = str(MAX_THREADS)
        index_type = 'opt'
        print >> sys.stderr, '\nBuilding %(index_type)s index' % locals()
        drop_caches()
        check_call((NUMACTL +

...
#include "accumulator/threshold_accumulator.hpp"
#include "app.hpp"
    ...
    bool safe)
{
    ...
    for (auto&& t: query_types) {
        ...
        if (t == "maxscore" && wand_data_filename) {
            query_fun = [&](Query query, Threshold t) {
                ThresholdAccumulator acc(t);
                maxscore_query maxscore_q(acc);
                maxscore_q(make_max_scored_cursors(index, wdata, *scorer, query), index.num_docs());
                acc.finalize();
                return acc.topk().size();
            };
        } else {
        ...
        if (extract) {
            ...
            op_perftest(query_fun, queries, thresholds, type, t, 2, k);
        }
    }
}
...
int main(int argc, const char** argv)
{
    ...
    App<arg::Index,
        ...
        arg::Scorer,
        arg::Thresholds>
        app{"Benchmark or extract times for queries that do not have top-k set but only rely on "
            "threshold to get the retults."};
    app.thresholds_option()->required(true);

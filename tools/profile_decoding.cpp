...
double measure_decoding_time(size_t sum_of_values, size_t n, std::vector<uint8_t> const& buf)
{
...
void profile_block(std::vector<uint32_t> const& values, uint32_t sum_of_values)
{
    ...
    for (uint8_t t = 0; t < mixed_block::block_types; ++t) {
        ...
        for (mixed_block::compr_param_type param = 0; param < mixed_block::compr_params(type);
             ++param) {
            buf.clear();
            if (!mixed_block::compression_stats(
                    type, param, values.data(), sum_of_values, n, buf, fv)) {
                continue;
            }
...
#define LOOP_BODY(R, DATA, T)                                         \
    ...
    {                                                                 \
        profile_decoding<BOOST_PP_CAT(T, _index)>(index_filename, p); \
        /**/

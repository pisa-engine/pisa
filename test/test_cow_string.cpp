...
TEST_CASE("CowString")
{
    GIVEN("An owned CowString")
    {
        ...
        WHEN("Accessed with as_view")
        {
            ...
            THEN("Equal to original value") { REQUIRE(view == value); }
            THEN("Data location is different") { REQUIRE(view.data() != value.data()); }
        }

        WHEN("Accessed with to_owned")
        {
            auto owned = std::move(cow).to_owned();
            THEN("Equal to original value") { REQUIRE(owned == value); }
            THEN("Data location is different") { REQUIRE(owned.data() != value.data()); }
        }
    }
    GIVEN("An owned CowString (moved from value)")
    {
        ...
        WHEN("Accessed with as_view")
        {
            auto view = cow.as_view();
            THEN("Equal to original value") { REQUIRE(view == VALUE); }
            THEN("Data location is the same as initially") { REQUIRE(view.data() == data_ptr); }
        }
        ...
        WHEN("Accessed with to_owned")
        {
            auto owned = std::move(cow).to_owned();
            THEN("Equal to original value") { REQUIRE(owned == VALUE); }
            THEN("Data location is the same as initially") { REQUIRE(owned.data() == data_ptr); }
        }
    }

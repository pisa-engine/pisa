...
TEST_CASE("Resolve shards", "[sharding]") {
    ...
    SECTION("With suffix") {
        for (auto f: std::vector<std::string>{"shard.000.docs", "shard.001.docs", "shard.002.docs"}) {
            std::ofstream os((dir.path() / f).string());
            os << ".";
        }
        REQUIRE(
...
TEST_CASE("create_random_mapping", "[invert][unit]") {
    ...
    REQUIRE(counts.as_vector() == std::vector<int>{77, 77, 77, 77, 77, 77, 77, 77, 77, 77, 77, 77, 76});
}

             ...
             std::string const& query_type)
{
    ...
    auto scorer_ptr = get_scorer("bm25", wdata);
    ...
    for (auto const& t: query_types) {
        ...
        if (t == "and") {
            query_fun = [&](Query query){
                and_query and_q;
                return and_q(make_cursors<typename add_profiling<IndexType>::type>(index, query), index.num_docs()).size();
            };
        } else if (t == "ranked_and" && wand_data_filename) {
            query_fun = [&](Query query){
                ranked_and_query ranked_and_q(10);
                return ranked_and_q(make_scored_cursors<typename add_profiling<IndexType>::type>(index, *scorer_ptr, query), index.num_docs());
            };
        } else if (t == "wand" && wand_data_filename) {
            query_fun = [&](Query query){
                wand_query wand_q(10);
                return wand_q(make_max_scored_cursors<typename add_profiling<IndexType>::type, WandType>(index, wdata, *scorer_ptr, query), index.num_docs());
            };
        } else if (t == "maxscore" && wand_data_filename) {
            query_fun = [&](Query query){
                maxscore_query maxscore_q(10);
                return maxscore_q(make_max_scored_cursors<typename add_profiling<IndexType>::type, WandType>(index, wdata, *scorer_ptr, query), index.num_docs());
            };
        } else {

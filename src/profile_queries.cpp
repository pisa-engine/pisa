    ...
    for (auto const &t : query_types) {
        ...
        if (t == "and") {
            query_fun = [&](Query query) {
                ...
                return and_q(make_scored_cursors<typename add_profiling<IndexType>::type>(
                                 index, wdata, bm25<WandType>(wdata), query),
                             index.num_docs())
                    .size();
            };
            ...
            query_fun = [&](Query query) {
                ...
                return ranked_and_q(
                    make_scored_cursors<typename add_profiling<IndexType>::type, WandType>(
                        index, wdata, bm25<WandType>(wdata), query),
                    index.num_docs());
            };

...
int main(int argc, const char **argv)
{
    ...
    bool compress = false;
    bool range = false;
    std::string drop_terms_filename;
    ...
    app.add_option("--drop-terms", drop_terms_filename, "A filename containing a list of term IDs that we want to drop");
    ...
    std::ifstream drop_terms_file(drop_terms_filename);
    std::unordered_set<size_t> drop_term_ids;
    copy(istream_iterator<size_t>(drop_terms_file),
         istream_iterator<size_t>(),
         inserter(drop_term_ids, drop_term_ids.end()));

    spdlog::info("Dropping {} terms", drop_term_ids.size());
    ...
    if (compress) {
        wand_data<wand_data_compressed> wdata(
            sizes_coll.begin()->begin(), coll.num_docs(), coll, block_size, drop_term_ids);
